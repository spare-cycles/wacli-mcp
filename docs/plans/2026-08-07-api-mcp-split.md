...
export type Route = {
   ...
   * other consumers, and a create that answers 200 is a small lie in the contract. Narrowed to the
   * three `implement()` can honestly write: it always writes a body, so a 204 would be a no-content
   * status carrying content.
   */
...
export type ImplementOptions = {
  /**
   * Check each JSON handler's result against its route's response schema before writing it. Off by
   * default: the client already `.parse`s on the way in, so a bad row is caught either way and the
   * only question is *where*. On, the failure is a 500 next to the handler that produced it; off,
   * it is a `ZodError` in the MCP one process away. Not simply on, because the API has consumers
   * other than this client and turning a response they accept today into a 500 is a behaviour
   * change nobody asked for. A development and test switch.
   */
...
export function createClient(opts: {
  ...
  /**
   * Where each request's `x-request-id` comes from. Defaults to `crypto.randomUUID`. A factory on
   * the client rather than a field on the call input, because `getHealth` declares nothing and so
   * takes no argument at all — an optional `requestId` would make `client.getHealth({})` compile,
   * which the contract pins as an error. Whatever it returns is also what the thrown `ApiError`
   * carries on `requestId`, which is what makes the two log streams joinable.
   */
...
- A `fetch` rejection (DNS, ECONNREFUSED, timeout) becomes `ApiUnreachableError`, never
  `NotConnectedError` — those two mean different things and the MCP surfaces them differently. So
  does a failure reading the *body*: `AbortSignal.timeout` aborts the body stream as well as the
  connect, and a truncated response rejects the same way, so both mean "no complete answer". The
  `ZodError` from the response schema is the one throw that stays a `ZodError` — that is the peer
  breaking the contract, not an unreachable API.
- `timeoutMs` uses `AbortSignal.timeout`, per route through `timeoutMsByRoute`.
- **Every request carries an `x-request-id` header**, generated by the client when absent. The split
  ...
  every request and every error. "When absent" is `requestIdFactory`: a caller with its own
  correlation id supplies it, and `crypto.randomUUID()` is the default. Every `ApiError` the client
  throws carries the id it sent on `requestId`, which is what makes the MCP side loggable at all —
  a header nobody can read back joins nothing. It costs a header and a `randomUUID()`; recovering
  the correlation after the fact costs an afternoon per incident.
