...
`wa_send_file`'s input is a Zod object with optional `path` and `data`, and exactly one must be present. A discriminated union would be cleaner in TypeScript but produces a JSON Schema that several MCP clients render poorly.

**Do not enforce that with `.refine()`.** On the pinned `@modelcontextprotocol/sdk@1.30.0` + `zod@3.25.76` it silently destroys the tool's advertised schema: `normalizeObjectSchema` identifies a v3 object solely by `.shape` (`server/zod-compat.js:105-113`), a `ZodEffects` has none, so `listTools` falls back to `EMPTY_OBJECT_JSON_SCHEMA` (`server/mcp.js:75-82`) and advertises `{"type":"object","properties":{}}` — **every argument, `chat` included, is invisible to every client.** Server-side validation still works, because `validateToolInput` falls back to the raw schema (`mcp.js:172-173`), so no behavioural test notices and the tool merely appears to take no arguments. Keep the shape flat and enforce "exactly one" in the handler. The same trap applies to any future tool reaching for `.refine()` or `.transform()`; Task 13 carries a test that walks every tool's advertised schema and fails on an empty `properties`. (Found by Task 13's implementer, reproduced end-to-end by review.)
