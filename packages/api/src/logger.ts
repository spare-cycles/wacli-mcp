...
const REDACT = ["token", "ntfy.token", "*.token", "authorization", "req.headers.authorization"];
 ...
 * it. That is not hypothetical here: body-parser attaches the entire raw request body, and an
 * undici- or axios-shaped failure from `downloadMediaMessage` carries the **WhatsApp CDN media
 * URL** — a capability URL that grants whoever holds it the attachment's encrypted bytes. Picking
 * the two fields explicitly keeps that true no matter what an error grows later.
 *
